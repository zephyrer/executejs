...
(function(){
   ...
   */
   ...
   var RijndaelEncryp = {
      encrpyt: function(plaintext, key, mode){
         if(key.indexOf("0x")==0)
            key = hexToByteArray(key)
         return rijndaelEncrypt(plaintext, key, mode)
      },

      decrypt: function(ciphertext, key, mode) {
         if(key.indexOf("0x")==0)
            key = hexToByteArray(key)
         return rijndaelDecrypt(ciphertext, key, mode)
      }
      
   }

   this.RijndaelEncryp = RijndaelEncryp;
}).apply(this)
}